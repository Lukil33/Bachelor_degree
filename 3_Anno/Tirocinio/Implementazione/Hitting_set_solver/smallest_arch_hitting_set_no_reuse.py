...
### Wrapper function
def complete_smallest_arch_no_reuse_hitting_set(connections: set) -> tuple[set, float]:

    start = time.perf_counter()

    # I calculate an euristical hitting set from the set of connections
    hitting_set = hitting_set_search(connections, set(), dict())
...
### Wrapper function
def partial_smallest_arch_no_reuse_hitting_set(connections: set, not_covered_arches: set, node_appearence: dict) -> tuple[set, float]:
    start = time.perf_counter()
...
def hitting_set_search(connections: set, not_covered_arches: set, node_appearence: dict) -> set:
    ...
    if not_covered_arches:
        # I sort the arches from the smallest to the largest degree 
        sorted_arches = sorted(not_covered_arches, key=lambda x: len(x))

        for arches in sorted_arches:
    ...
    # I sort the arches from the smallest to the largest degree 
    sorted_arches = sorted(connections, key=lambda x: len(x))
