...
### Wrapper function
def complete_biggest_arch_hitting_set(connections: set) -> tuple[set, float]:
...
### Wrapper function
def partial_biggest_arch_hitting_set(connections: set, not_covered_arches: set, node_appearence: dict) -> tuple[set, float]:
    start = time.perf_counter()
...
def hitting_set_search(connections: set, not_covered_arches: set, node_appearence: dict) -> set:
    ...
    if not_covered_arches:
        # I sort the arches from the biggest to the smallest degree
        sorted_arches = sorted(connections, key=lambda x: len(x), reverse=True)
